rvQCA<-function(qca.data, outcome="OUT", conditions=c(""),sim=100, ncut=4){
  ptm <- proc.time()
  ...
  rows<-sim*length(ncut)*length(inclcut)*length(pop)*2*length(conditions)* length(seq(from=10,  to=150, by=10)) #total number of rows

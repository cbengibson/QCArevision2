...
#' 
#' This function performs the the Irvine Assessment for QCA (iaQCA) on a given QCA model object.
#' @import QCA bootstrap
#' @param mod name of the QCA eqmcc model object.
#' @param sim the number of simulations the iaQCA function should run. Default set to \code{sim=2000}.
...
#' iaQCA(mod1) 
#' @export
iaQCA<-function(mod, sim=2000, include=c(""), row.dom=F, omit=c(), dir.exp=c() ){
  ptm <- proc.time()

...
#logistic regression predicting probability of returning a spurious relationship
  if (length(ncut)==1){suppressWarnings(mod<-glm(OUT ~ CTH + CPI, family="binomial", data=data))}
  if (length(ncut)>2){suppressWarnings(mod<-glm(OUT ~ CTH + CNTH + CPI, family="binomial", data=data))}
  
  
data$pred<-predict(mod,data, type="response")

data<- cbind(data, predict(mod, newdata = data, type = "link", se = TRUE))

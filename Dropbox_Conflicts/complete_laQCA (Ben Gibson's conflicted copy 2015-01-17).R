####FREQUENTIST ROBUSTNESS ASSESSMENT FOR QCA INSPIRED BY THE GREAT STANLEY LIEBERSON YALL ROLL TIDE####
...
ltQCA<-function(mod, sim=2000, verbose=T, include=c(""), row.dom=F, all.sol=F, omit=c(), dir.exp=c(), conv.diag=F  ){
  ...
  s.qca.data<-do.call("list", replicate(sim, qca.data, simplify = FALSE)) #create a list of "zero-matrices" to pre-allocate memory in later steps
  ...
  for (j in 1:sim) {   #for j in 1:how many simulations the researcher specified (default is 2000 simulations)
    ...
    if (type=="crisp"){ #have to specify: am 

...
%The Irvine Robustness Assessment: a Hypothesis Test for Qualitative Comparative Analysis (QCA)
%opening
...
\begin{table}[h] %missing [] == [t], h is current position
\caption{QCA Results for Florida Tea Party Rallies, After Applying iaQCA} \label{tab:title} 
\begin{center}
%\hspace*{-.5in}
\begin{tabular}{ >{$}l<{$}  >{$}c<{$} >{$}c<{$} >{$}c<{$}}
  \text{Solutions} &  & \text{Consistency} & \text{Coverage} \\
...
%Finally, we provide the software to achieve a Irvine Assessment in the Comprehensive R Archive Network (CRAN) under package iaQCA. 
